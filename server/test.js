...
function makeCard(src, title, text, btnClass = 'btn btn-outline') {
  ...
  try {
    if (
      ...
      c.template.children[0].attributes
    ) {
    ...
    if (
      c.template &&
      c.template.children &&
      c.template.children[1] &&
      c.template.children[1].children
    ) {
...
const sampleConfigs = {
  ...
  cards: [
    makeCard(
      ...
      'カードタイトル1',
      'カードの説明文です。JSONから生成されたカードコンポーネント。',
      'btn btn-outline'
    ),
    makeCard(
      'https://via.placeholder.com/300x200',
      'カードタイトル2',
      '別のカードの説明文です。同じ構造で異なる内容。',
      'btn btn-outline'
    ),
  ],
};
