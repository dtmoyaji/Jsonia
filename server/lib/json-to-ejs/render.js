      ...
      if (fs.existsSync(behaviorPath)) {
        try {
          ...
          if (!options.collectedBehaviors.find((b) => b.__source === behaviorPath)) {
            options.collectedBehaviors.push({
              __source: behaviorPath,
              name: compName,
...
function renderPage(pageConfig, options = {}) {
  ...
  const bodyEjs =
    body.length > 0 ? '    ' + render(body, options).replace(/\n/g, '\n    ') + '\n' : '';
      ...
      if (style.$include) {
        try {
          ...
          if (fullPath) {
            ...
            const mediaAttr = style.media
              ? ` media="${utils.escapeHtml(String(style.media))}"`
              : '';
            ejs += `    <style${mediaAttr}>\n${cssText}\n    </style>\n`;
          } else {
  ...
  if (options.collectedStyles && options.collectedStyles.length > 0) {
    for (const styleObj of options.collectedStyles) {
      try {
        ...
        ejs += `    <style data-source="${utils.escapeHtml(
          styleObj.__source || ''
        )}">\n${cssText}\n    </style>\n`;
      } catch (err) {
  ...
  if (options.collectedBehaviors && options.collectedBehaviors.length > 0) {
    try {
      const payload = JSON.stringify(
        options.collectedBehaviors.map((b) => ({ name: b.name, behavior: b.behavior }))
      );
      ejs += `    <script>\n`;
      ejs += `        (function(){\n`;
      ...
      ejs += `    <!-- Failed to inject collected behaviors: ${utils.escapeHtml(
        err.message
      )} -->\n`;
    }
  }
