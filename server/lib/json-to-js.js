    ...
    const apiMethods = Object.entries(apisConfig)
      .map(([name, config]) => {
        ...
        try {
            ...
            ${
              method !== 'GET'
                ? `options.body = JSON.stringify(${body.replace(
                    /\{\{([^}]+)\}\}/g,
                    (_, path) => `params.${path}`
                  )});`
                : ''
      ...
      .map((action) => {
        switch (action.type) {
          ...
          case 'updateDOM': {
            ...
            return `            document.querySelector('${selector}').innerHTML = ${JSON.stringify(
              content
            )};`;
          }
  ...
  generateValidation(validationConfig) {
    const validators = Object.entries(validationConfig)
      .map(([field, rules]) => {
        ...
        if (rules.type === 'email') {
          checks.push(
            `if (!/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(value)) return { valid: false, message: '${
              rules.message || '有効なメールアドレスを入力してください'
            }' };`
          );
        }
        ...
        if (rules.minLength) {
          checks.push(
            `if (value.length < ${rules.minLength}) return { valid: false, message: '${
              rules.message || field + 'は' + rules.minLength + '文字以上で入力してください'
            }' };`
          );
        }
        ...
        if (rules.maxLength) {
          checks.push(
            `if (value.length > ${rules.maxLength}) return { valid: false, message: '${
              rules.message || field + 'は' + rules.maxLength + '文字以内で入力してください'
            }' };`
          );
        }
        ...
        if (rules.pattern) {
          checks.push(
            `if (!new RegExp('${rules.pattern}').test(value)) return { valid: false, message: '${
              rules.message || field + 'の形式が正しくありません'
            }' };`
          );
        }
